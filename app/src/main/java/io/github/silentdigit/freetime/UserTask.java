...
public class UserTask implements Comparable<UserTask>, Parcelable {

    private String taskName;
    private Location taskLocation;
    private String taskLocationQuery;
    private String taskAddress;
    private String taskLatLong;
    private Integer taskDistance;
    private String taskDistanceString;
    private String taskTime;
    private TransitDataTask taskUpdate;

    UserTask(String name, String location, Location currentLocation, Activity activity) {
        taskName = name;
        ...
        try {
            ...
            if (taskAddresses != null && taskAddresses.size() > 0) {
                ...
                taskAddress = address.getAddressLine(0); // + String.format(Locale.getDefault(), "%.3f, %.3f", address.getLatitude(), address.getLongitude());
                taskLatLong = String.format(Locale.getDefault(), "%.3f, %.3f", address.getLatitude(), address.getLongitude());
...
//                Log.i("DestInfo", destinationAddress.toString());
    ...
    public String getTaskLocationQuery() {
        return taskLocationQuery;
    }
...
//            dist = String.format(Locale.getDefault(), "Distance: %.1f km", estimate);
    ...
    protected UserTask(Parcel in) {
        ...
        taskLocation = (Location) in.readValue(Location.class.getClassLoader());
        taskLocationQuery = in.readString();
        taskAddress = in.readString();
        taskLatLong = in.readString();
        taskDistance = in.readByte() == 0x00 ? null : in.readInt();
        taskDistanceString = in.readString();
    ...
    public void writeToParcel(Parcel dest, int flags) {
        ...
        dest.writeValue(taskLocation);
        dest.writeString(taskLocationQuery);
        dest.writeString(taskAddress);
        dest.writeString(taskLatLong);
        if (taskDistance == null) {
            dest.writeByte((byte) (0x00));
        } else {
            dest.writeByte((byte) (0x01));
            dest.writeInt(taskDistance);
        }
        dest.writeString(taskDistanceString);
